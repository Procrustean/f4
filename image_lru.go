...
// entries; otherwise the limit is a byte budget. Every method must be called
// with the pipeline lock held.
...
// least recently used entries while over the budget. The newest entry always
// survives, so the picture on screen is kept even when it alone is larger
